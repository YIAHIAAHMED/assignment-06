import React from 'react';
import createImg from '../assets/user.png'

const Steps = () => {
    return (
        <div className='w-full max-w-7xl mx-auto h-[754px]'>
            <div className="flex flex-col items-center">
                <h2 className='text-5xl'>Get Started in 3 Steps</h2>
                <p className='text-base text-[#627382FF] '>Start using premium digital tools in minutes, not hours.</p>
            </div>
            <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-4">
                {/* 01 */}
                <div className="border">
                    <div className="">
                        <div className="flex justify-end">
                            <button className='btn btn-primary w-12 h-12 rounded-full bg-gradient-to-r from-[#4F39F6] to-[#9514FA] text-right'>01 </button>
                        </div>
                        <div className="flex flex-col justify-center items-center">
                            <div className="w-16 h-16 rounded-full bg-[#F3E8FF] items-center flex justify-center">
                                <img className='bg-[#ffffff00] ' src={createImg} alt='user' />
                            </div>
                            <h2 className='text-2xl'>Create Account</h2>
                            <p className='text-base text-[#627382FF] '>Sign up for free in seconds. No credit card required to get started.</p>
                        </div>
                    </div>


                </div>

                {/* 02 */}
                <div className="border">
                    <div className="">
                        <div className="flex justify-end">
                            <button className='btn btn-primary w-12 h-12 rounded-full bg-gradient-to-r from-[#4F39F6] to-[#9514FA] text-right'>01 </button>
                        </div>
                        <div className="flex flex-col justify-center items-center">
                            <div className="w-16 h-16 rounded-full bg-[#F3E8FF] items-center flex justify-center">
                                <img className='bg-[#ffffff00] ' src={createImg} alt='user' />
                            </div>
                            <h2 className='text-2xl'>Create Account</h2>
                            <p className='text-base text-[#627382FF] '>Sign up for free in seconds. No credit card required to get started.</p>
                        </div>
                    </div>


                </div>


                {/* 03 */}
                <div className="border">
                    <div className="">
                        <div className="flex justify-end">
                            <button className='btn btn-primary w-12 h-12 rounded-full bg-gradient-to-r from-[#4F39F6] to-[#9514FA] text-right'>01 </button>
                        </div>
                        <div className="flex flex-col justify-center items-center">
                            <div className="w-16 h-16 rounded-full bg-[#F3E8FF] items-center flex justify-center">
                                <img className='bg-[#ffffff00] ' src={createImg} alt='user' />
                            </div>
                            <h2 className='text-2xl'>Create Account</h2>
                            <p className='text-base text-[#627382FF] '>Sign up for free in seconds. No credit card required to get started.</p>
                        </div>
                    </div>


                </div>
            </div>

        </div>
    );
};

export default Steps;